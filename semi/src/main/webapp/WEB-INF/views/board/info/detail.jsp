...
<div class="container w-800">
	...
	<div class="cell right" style="margin-top: 10px;">
		<i id="board-like" class="fa-regular fa-thumbs-up"
			style="font-size: 1.8rem; cursor: pointer; color: #a67c52;"></i> <span
			id="board-like-count"
	...
	$(function() {
		...
		$.ajax({
			...
			success : function(response) {
				if (response.like) {
					$("#board-like").removeClass("fa-regular").addClass(
							"fa-solid");
				} else {
					$("#board-like").removeClass("fa-solid").addClass(
							"fa-regular");
				}
...
<%-- 로그인 아이디가 없으면 좋아요 클릭 막기 위해 flag 생성 --%>
	var isLoggedIn =
<%=(session.getAttribute("loginId") != null ? "true" : "false")%>
	;
			...
			$("#board-like").on(
					...
					function() {
						$.ajax({
							...
							success : function(response) {
								if (response.like) {
									$("#board-like").removeClass("fa-regular")
											.addClass("fa-solid");
								} else {
									$("#board-like").removeClass("fa-solid")
											.addClass("fa-regular");
								}
								$("#board-like-count").text(response.count);
							},
