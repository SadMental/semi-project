...
public class TypeHeaderDao {
    ...
    public boolean insert(TypeHeaderDto dto) {
        String sql = "INSERT INTO type_header (type_header_no, type_header_name) VALUES (?, ?)";
        Object[] params = { dto.getTypeHeaderNo(), dto.getTypeHeaderName() };
        return jdbcTemplate.update(sql, params) > 0;
    }

    /** 수정 (UPDATE) */
    public boolean update(TypeHeaderDto dto) {
        String sql = "UPDATE type_header SET type_header_name = ? WHERE type_header_no = ?";
        Object[] params = { dto.getTypeHeaderName(), dto.getTypeHeaderNo() };
        return jdbcTemplate.update(sql, params) > 0;
    }
    ...
    public boolean delete(int no) {
        String sql = "DELETE FROM type_header WHERE type_header_no = ?";
        Object[] params = { no };
        return jdbcTemplate.update(sql, params) > 0;
    }
    ...
    public List<TypeHeaderDto> selectByName(String name) {
        String sql = "SELECT * FROM type_header WHERE type_header_name = ?";
        Object[] params = { name };
